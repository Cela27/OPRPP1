...
/**
 * Class which represents linked list-backed collection of objects. You can
...
public class LinkedListIndexedCollection extends Collection {
	...
	*/
	...
	/**
	 * Constructor for class, you can choose desired collection whose items will be part of it 
	...
	public LinkedListIndexedCollection(Collection collection) {
		...
		first=new ListNode(collection.get(0));
		ListNode current= first;
		
		for (int i = 0; i < collection.size()-1; i++) {
			current.setContent(collection.get(i));
			ListNode next=new ListNode(collection.get(i+1));
	...
	*/
	...
	public void forEach(Processor processor) {
		ListNode current=first;
		for(int i=0; i<size;i++) {
			processor.process(current.getContent());
			current= current.getNext();
		}
	}
